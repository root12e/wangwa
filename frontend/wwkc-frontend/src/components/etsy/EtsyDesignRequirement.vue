<template>
  <div class="etsy-design-requirement">
    <!-- Redis缓存状态监控 -->
    <RedisStatusMonitor 
    ...
    <div class="component-header">
      ...
      <div class="header-actions">
        ...
        <el-button type="success" @click="showDesignProgressView = true">
          <el-icon><TrendCharts /></el-icon>
          设计进度
        </el-button>
        <el-button type="warning" @click="showDesignerWorkloadView = true">
    ...
    <div class="filter-section">
      <el-form :model="filterForm" inline>
        ...
        <el-form-item label="状态指示">
          <el-select v-model="filterForm.status_indication" placeholder="选择状态" clearable>
        ...
        <el-form-item label="排序字段">
          <el-select v-model="filterForm.sort_by" placeholder="选择排序字段">
            ...
            <el-option label="状态指示" value="status_indication" />
    ...
    >
      ...
      <el-table-column prop="status_indication" label="状态指示" width="100">
        <template #default="{ row }">
          <el-tag :type="getStatusType(row.status_indication)">
            {{ row.status_indication }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="design_deadline" label="设计截止日期" width="120" />
            ...
            >
              <div class="virtual-row-content">
                ...
                <div class="row-cell" style="width: 100px;">
                  <el-tag :type="getStatusType(item.status_indication)">
                    {{ item.status_indication }}
                  </el-tag>
                </div>
                <div class="row-cell" style="width: 100px;">{{ item.attribute }}</div>
                <div class="row-cell" style="width: 100px;">
                  <el-tag :type="getPriorityType(item.priority)">
                    {{ item.priority }}
                  </el-tag>
                </div>
                <div class="row-cell" style="width: 100px;">{{ item.designer }}</div>
                <div class="row-cell" style="width: 120px;">{{ item.design_deadline }}</div>
    ...
    <el-dialog
      ...
      :title="editingItem ? '编辑设计需求' : '新增设计需求'"
      width="900px"
    >
      <el-form
        ref="formRef"
      ...
      >
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="状态指示" prop="status_indication">
              <el-select v-model="formData.status_indication" placeholder="选择状态" style="width: 100%">
                <el-option label="待设计" value="待设计" />
                <el-option label="设计中" value="设计中" />
                <el-option label="已完成" value="已完成" />
                <el-option label="已取消" value="已取消" />
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="属性" prop="attribute">
              <el-input v-model="formData.attribute" placeholder="请输入产品属性" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="设计师" prop="designer">
              <el-input v-model="formData.designer" placeholder="请输入设计师姓名" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="设计截止日期" prop="design_deadline">
              <el-date-picker
                v-model="formData.design_deadline"
                type="date"
                placeholder="选择截止日期"
                format="YYYY-MM-DD"
                value-format="YYYY-MM-DD"
        ...
        <el-form-item label="设计说明" prop="design_description">
          <el-input
            v-model="formData.design_description"
            type="textarea"
            :rows="3"
            placeholder="请输入设计说明"
          />
...
import { etsyDesignRequirementAPI } from '@/api/etsy'
...
const fetchData = async () => {
  try {
    ...
    const response = await etsyDesignRequirementAPI.getList(params)
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除这个设计需求吗？', '提示', {
      type: 'warning'
    })
    
    await etsyDesignRequirementAPI.delete(row.id)
...
const handleSubmit = async () => {
  try {
    ...
    if (editingItem.value) {
      await etsyDesignRequirementAPI.update(editingItem.value.id, formData)
      ElMessage.success('更新成功')
    } else {
      await etsyDesignRequirementAPI.create(formData)
...
const handleBulkDelete = async () => {
  try {
    await ElMessageBox.confirm(`确定要删除选中的 ${selectedRows.value.length} 个设计需求吗？`, '提示', {
      type: 'warning'
    })
    
    const ids = selectedRows.value.map(row => row.id)
    await etsyDesignRequirementAPI.bulkDelete({ ids })
...
const handleBulkUpdate = () => {
  if (selectedRows.value.length === 0) {
    ElMessage.warning('请先选择要更新的设计需求')
    return
  }
...
const confirmBulkUpdate = async () => {
  try {
    ...
    await etsyDesignRequirementAPI.bulkUpdate({ ids, updates })
...
const downloadTemplate = async () => {
  try {
    const response = await etsyDesignRequirementAPI.downloadTemplate()
    const blob = new Blob([response.data])
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = '设计需求表模板.xlsx'
    link.click()
...
const handleImport = async () => {
  ...
  try {
    ...
    await etsyDesignRequirementAPI.importData(file)
...
const handleSyncData = async () => {
  try {
    await etsyDesignRequirementAPI.syncData()
...
const handleClearAllCache = async () => {
  try {
    await etsyDesignRequirementAPI.clearCache()
