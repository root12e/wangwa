<template>
  <div class="etsy-production-requirement">
    <!-- Redis缓存状态监控 -->
    <RedisStatusMonitor 
    ...
    <div class="component-header">
      ...
      <div class="header-actions">
        ...
        <el-button type="success" @click="showProductionProgressView = true">
          <el-icon><TrendCharts /></el-icon>
          生产进度
        </el-button>
        <el-button type="warning" @click="showProductionCapacityView = true">
    ...
    >
      ...
      <el-table-column prop="requirement_number" label="需求编号" width="150" />
          ...
          >
            ...
            >
              <div class="virtual-row-content">
                <div class="row-cell" style="width: 150px;">{{ item.requirement_number }}</div>
                ...
                <div class="row-cell" style="width: 120px;">{{ item.deadline }}</div>
    ...
    <el-dialog
      ...
      :title="editingItem ? '编辑生产需求' : '新增生产需求'"
      width="800px"
    >
      <el-form
        ref="formRef"
...
import { Plus, Upload, Download, UploadFilled, View, Refresh, Setting } from '@element-plus/icons-vue'
import { etsyProductionRequirementAPI } from '@/api/etsy'
...
const fetchData = async () => {
  try {
    ...
    const response = await etsyProductionRequirementAPI.getList(params)
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除这个生产需求吗？', '提示', {
      type: 'warning'
    })
    
    await etsyProductionRequirementAPI.delete(row.id)
...
const handleSubmit = async () => {
  try {
    ...
    if (editingItem.value) {
      await etsyProductionRequirementAPI.update(editingItem.value.id, formData)
      ElMessage.success('更新成功')
    } else {
      await etsyProductionRequirementAPI.create(formData)
...
const handleBulkDelete = async () => {
  try {
    await ElMessageBox.confirm(`确定要删除选中的 ${selectedRows.value.length} 个生产需求吗？`, '提示', {
      type: 'warning'
    })
    
    const ids = selectedRows.value.map(row => row.id)
    await etsyProductionRequirementAPI.bulkDelete({ ids })
...
const handleBulkUpdate = () => {
  if (selectedRows.value.length === 0) {
    ElMessage.warning('请先选择要更新的生产需求')
    return
  }
...
const confirmBulkUpdate = async () => {
  try {
    ...
    await etsyProductionRequirementAPI.bulkUpdate({ ids, updates })
...
const downloadTemplate = async () => {
  try {
    const response = await etsyProductionRequirementAPI.downloadTemplate()
    const blob = new Blob([response.data])
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = '生产需求表模板.xlsx'
    link.click()
...
const handleImport = async () => {
  ...
  try {
    ...
    await etsyProductionRequirementAPI.importData(file)
...
const handleSyncData = async () => {
  try {
    await etsyProductionRequirementAPI.syncData()
...
const handleClearAllCache = async () => {
  try {
    await etsyProductionRequirementAPI.clearCache()
