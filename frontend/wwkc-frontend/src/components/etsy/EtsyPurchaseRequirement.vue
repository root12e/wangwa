<template>
  <div class="etsy-purchase-requirement">
    <!-- Redis缓存状态监控 -->
    <RedisStatusMonitor 
    ...
    <div class="component-header">
      ...
      <div class="header-actions">
        ...
        <el-button type="success" @click="showPurchaseProgressView = true">
          <el-icon><TrendCharts /></el-icon>
          采购进度
        </el-button>
        <el-button type="warning" @click="showSupplierAnalysisView = true">
          ...
          >
            ...
            >
              <div class="virtual-row-content">
                ...
                <div class="row-cell" style="width: 120px;">{{ item.supplier }}</div>
...
import { Plus, Upload, Download, UploadFilled, View, Refresh, Shop } from '@element-plus/icons-vue'
import { etsyPurchaseRequirementAPI } from '@/api/etsy'
...
const fetchData = async () => {
  try {
    ...
    const response = await etsyPurchaseRequirementAPI.getList(params)
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除这个采购需求吗？', '提示', {
      type: 'warning'
    })
    
    await etsyPurchaseRequirementAPI.delete(row.id)
...
const handleSubmit = async () => {
  try {
    ...
    if (editingItem.value) {
      await etsyPurchaseRequirementAPI.update(editingItem.value.id, formData)
      ElMessage.success('更新成功')
    } else {
      await etsyPurchaseRequirementAPI.create(formData)
...
const handleBulkDelete = async () => {
  try {
    await ElMessageBox.confirm(`确定要删除选中的 ${selectedRows.value.length} 个采购需求吗？`, '提示', {
      type: 'warning'
    })
    
    const ids = selectedRows.value.map(row => row.id)
    await etsyPurchaseRequirementAPI.bulkDelete({ ids })
...
const handleBulkUpdate = () => {
  if (selectedRows.value.length === 0) {
    ElMessage.warning('请先选择要更新的采购需求')
    return
  }
...
const confirmBulkUpdate = async () => {
  try {
    ...
    await etsyPurchaseRequirementAPI.bulkUpdate({ ids, updates })
...
const downloadTemplate = async () => {
  try {
    const response = await etsyPurchaseRequirementAPI.downloadTemplate()
    const blob = new Blob([response.data])
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = '采购需求表模板.xlsx'
    link.click()
...
const handleImport = async () => {
  ...
  try {
    ...
    await etsyPurchaseRequirementAPI.importData(file)
...
const handleSyncData = async () => {
  try {
    await etsyPurchaseRequirementAPI.syncData()
...
const handleClearAllCache = async () => {
  try {
    await etsyPurchaseRequirementAPI.clearCache()
