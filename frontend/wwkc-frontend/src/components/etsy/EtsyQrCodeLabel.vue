  ...
  <div class="etsy-qr-code-label">
    ...
    <div class="component-header">
      ...
      <div class="header-actions">
        ...
        <el-button type="success" @click="showBatchGenerateView = true">
          <el-icon><MagicStick /></el-icon>
          批量生成
        </el-button>
        <el-button type="warning" @click="showLabelAnalysisView = true">
    ...
    <div class="filter-section">
      <el-form :model="filterForm" inline>
        ...
        <el-form-item label="标签类型">
          <el-select v-model="filterForm.label_type" placeholder="选择类型" clearable>
            <el-option label="产品标签" value="产品标签" />
            <el-option label="包装标签" value="包装标签" />
            <el-option label="运输标签" value="运输标签" />
          </el-select>
        </el-form-item>
    ...
    <div class="bulk-actions" v-if="selectedRows.length > 0">
      ...
      <el-button @click="handleBulkPrint">
    ...
    >
      ...
      <el-table-column prop="label_number" label="标签编号" width="150" />
      ...
      <el-table-column prop="label_type" label="标签类型" width="100">
        <template #default="{ row }">
          <el-tag :type="getLabelTypeType(row.label_type)">
            {{ row.label_type }}
          </el-tag>
        </template>
      </el-table-column>
          ...
          >
            ...
            >
              <div class="virtual-row-content">
                <div class="row-cell" style="width: 150px;">{{ item.label_number }}</div>
                ...
                <div class="row-cell" style="width: 100px;">
                  <el-tag :type="getLabelTypeType(item.label_type)">
                    {{ item.label_type }}
                  </el-tag>
                </div>
    ...
    <el-dialog
      ...
      :title="editingItem ? '编辑二维码标签' : '新增二维码标签'"
      width="800px"
    >
      <el-form
        ref="formRef"
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="标签类型" prop="label_type">
              <el-select v-model="formData.label_type" placeholder="选择标签类型" style="width: 100%">
                <el-option label="产品标签" value="产品标签" />
                <el-option label="包装标签" value="包装标签" />
                <el-option label="运输标签" value="运输标签" />
              </el-select>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="二维码内容" prop="qr_content">
              <el-input v-model="formData.qr_content" placeholder="请输入二维码内容" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="标签尺寸" prop="label_size">
              <el-select v-model="formData.label_size" placeholder="选择标签尺寸" style="width: 100%">
                <el-option label="小号" value="小号" />
                <el-option label="中号" value="中号" />
      ...
      <el-form :model="bulkUpdateForm" label-width="120px">
        <el-form-item label="更新字段">
          <el-select v-model="bulkUpdateForm.field" placeholder="选择要更新的字段" style="width: 100%">
            ...
            <el-option label="标签类型" value="label_type" />
...
import { Plus, Upload, Download, UploadFilled, View, Refresh, MagicStick, TrendCharts } from '@element-plus/icons-vue'
import { etsyQrCodeLabelAPI } from '@/api/etsy'
...
const fetchData = async () => {
  try {
    ...
    const response = await etsyQrCodeLabelAPI.getList(params)
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除这个二维码标签吗？', '提示', {
      type: 'warning'
    })
    
    await etsyQrCodeLabelAPI.delete(row.id)
...
const handlePrint = async (row: any) => {
  try {
    await etsyQrCodeLabelAPI.print(row.id)
...
const handleSubmit = async () => {
  try {
    ...
    if (editingItem.value) {
      await etsyQrCodeLabelAPI.update(editingItem.value.id, formData)
      ElMessage.success('更新成功')
    } else {
      await etsyQrCodeLabelAPI.create(formData)
...
const handleBulkDelete = async () => {
  try {
    await ElMessageBox.confirm(`确定要删除选中的 ${selectedRows.value.length} 个二维码标签吗？`, '提示', {
      type: 'warning'
    })
    
    const ids = selectedRows.value.map(row => row.id)
    await etsyQrCodeLabelAPI.bulkDelete({ ids })
...
const handleBulkPrint = async () => {
  try {
    const ids = selectedRows.value.map(row => row.id)
    await etsyQrCodeLabelAPI.bulkPrint({ ids })
...
const handleBulkUpdate = () => {
  if (selectedRows.value.length === 0) {
    ElMessage.warning('请先选择要更新的二维码标签')
    return
  }
...
const confirmBulkUpdate = async () => {
  try {
    ...
    await etsyQrCodeLabelAPI.bulkUpdate({ ids, updates })
...
const downloadTemplate = async () => {
  try {
    const response = await etsyQrCodeLabelAPI.downloadTemplate()
    const blob = new Blob([response.data])
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = '二维码标签表模板.xlsx'
    link.click()
...
const handleImport = async () => {
  ...
  try {
    ...
    await etsyQrCodeLabelAPI.importData(file)
...
// 获取标签类型标签类型
const getLabelTypeType = (labelType: string) => {
  const labelTypeMap: Record<string, string> = {
    '产品标签': 'primary',
    '包装标签': 'success',
    '运输标签': 'warning'
  }
  return labelTypeMap[labelType] || 'info'
}

// Redis缓存相关方法
const handleSyncData = async () => {
  try {
    await etsyQrCodeLabelAPI.syncData()
...
const handleClearAllCache = async () => {
  try {
    await etsyQrCodeLabelAPI.clearCache()
