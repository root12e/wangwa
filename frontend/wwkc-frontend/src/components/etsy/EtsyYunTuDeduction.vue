<template>
  <div class="etsy-yuntu-deduction">
    <!-- Redis缓存状态监控 -->
    <RedisStatusMonitor 
    ...
    <div class="component-header">
      ...
      <div class="header-actions">
        ...
        <el-button type="success" @click="showDeductionAnalysisView = true">
          <el-icon><TrendCharts /></el-icon>
          扣费分析
        </el-button>
        <el-button type="warning" @click="showCostControlView = true">
      ...
      <el-form :model="filterForm" inline>
        ...
        <el-form-item label="扣费状态">
          <el-select v-model="filterForm.status" placeholder="选择状态" clearable>
            <el-option label="待扣费" value="待扣费" />
            <el-option label="已扣费" value="已扣费" />
            <el-option label="扣费失败" value="扣费失败" />
            <el-option label="已退款" value="已退款" />
          </el-select>
        </el-form-item>
        <el-form-item label="扣费类型">
          <el-select v-model="filterForm.deduction_type" placeholder="选择类型" clearable>
            <el-option label="图片扣费" value="图片扣费" />
            <el-option label="视频扣费" value="视频扣费" />
            <el-option label="模板扣费" value="模板扣费" />
          </el-select>
        </el-form-item>
    ...
    <div class="bulk-actions" v-if="selectedRows.length > 0">
      ...
      <el-button @click="handleBulkDeduct">
        批量扣费 ({{ selectedRows.length }})
      </el-button>
    ...
    >
      ...
      <el-table-column prop="deduction_number" label="扣费编号" width="150" />
      <el-table-column prop="sku" label="SKU" width="120" />
      <el-table-column prop="product_name" label="产品名称" min-width="150" />
      <el-table-column prop="store" label="店铺" width="120" />
      <el-table-column prop="status" label="扣费状态" width="100">
      ...
      <el-table-column prop="deduction_type" label="扣费类型" width="100">
        <template #default="{ row }">
          <el-tag :type="getDeductionTypeType(row.deduction_type)">
            {{ row.deduction_type }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="amount" label="扣费金额" width="120">
        <template #default="{ row }">
          ¥{{ row.amount }}
        </template>
      </el-table-column>
      <el-table-column prop="deduction_date" label="扣费日期" width="120" />
      <el-table-column prop="created_at" label="创建时间" width="150" />
      <el-table-column label="操作" width="250" fixed="right">
        <template #default="{ row }">
          <el-button size="small" @click="handleEdit(row)">编辑</el-button>
          <el-button size="small" type="success" @click="handleDeduct(row)">扣费</el-button>
          ...
          >
            ...
            >
              <div class="virtual-row-content">
                <div class="row-cell" style="width: 150px;">{{ item.deduction_number }}</div>
                ...
                <div class="row-cell" style="width: 100px;">
                  <el-tag :type="getDeductionTypeType(item.deduction_type)">
                    {{ item.deduction_type }}
                  </el-tag>
                </div>
                <div class="row-cell" style="width: 120px;">¥{{ item.amount }}</div>
    ...
    <el-dialog
      ...
      :title="editingItem ? '编辑云图扣费' : '新增云图扣费'"
      width="800px"
    >
      <el-form
        ref="formRef"
      ...
      >
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="扣费类型" prop="deduction_type">
              <el-select v-model="formData.deduction_type" placeholder="选择扣费类型" style="width: 100%">
                <el-option label="图片扣费" value="图片扣费" />
                <el-option label="视频扣费" value="视频扣费" />
                <el-option label="模板扣费" value="模板扣费" />
              </el-select>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="扣费日期" prop="deduction_date">
              <el-date-picker
          ...
          <el-col :span="12">
            <el-form-item label="资源数量" prop="resource_count">
              <el-input-number 
                v-model="formData.resource_count" 
                :min="1" 
                placeholder="请输入资源数量"
                style="width: 100%"
              />
        ...
        <el-form-item label="扣费原因" prop="deduction_reason">
          <el-input
            v-model="formData.deduction_reason"
            type="textarea"
            :rows="3"
            placeholder="请输入扣费原因"
          />
      ...
      <el-form :model="bulkUpdateForm" label-width="120px">
        <el-form-item label="更新字段">
          <el-select v-model="bulkUpdateForm.field" placeholder="选择要更新的字段" style="width: 100%">
            <el-option label="扣费状态" value="status" />
            <el-option label="扣费类型" value="deduction_type" />
...
import { etsyYunTuDeductionAPI } from '@/api/etsy'
...
const formRules = {
  ...
  deduction_type: [{ required: true, message: '请选择扣费类型', trigger: 'change' }],
  status: [{ required: true, message: '请选择扣费状态', trigger: 'change' }],
  amount: [{ required: true, message: '请输入扣费金额', trigger: 'blur' }]
}
...
const fetchData = async () => {
  try {
    ...
    const response = await etsyYunTuDeductionAPI.getList(params)
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除这个云图扣费记录吗？', '提示', {
      type: 'warning'
    })
    
    await etsyYunTuDeductionAPI.delete(row.id)
...
// 扣费
const handleDeduct = async (row: any) => {
  try {
    await etsyYunTuDeductionAPI.deduct(row.id)
...
const handleSubmit = async () => {
  try {
    ...
    if (editingItem.value) {
      await etsyYunTuDeductionAPI.update(editingItem.value.id, formData)
      ElMessage.success('更新成功')
    } else {
      await etsyYunTuDeductionAPI.create(formData)
...
const handleBulkDelete = async () => {
  try {
    await ElMessageBox.confirm(`确定要删除选中的 ${selectedRows.value.length} 个云图扣费记录吗？`, '提示', {
      type: 'warning'
    })
    
    const ids = selectedRows.value.map(row => row.id)
    await etsyYunTuDeductionAPI.bulkDelete({ ids })
...
const handleBulkDeduct = async () => {
  try {
    const ids = selectedRows.value.map(row => row.id)
    await etsyYunTuDeductionAPI.bulkDeduct({ ids })
    ElMessage.success('批量扣费成功')
    selectedRows.value = []
    fetchData()
  } catch (error) {
...
const handleBulkUpdate = () => {
  if (selectedRows.value.length === 0) {
    ElMessage.warning('请先选择要更新的云图扣费记录')
    return
  }
...
const confirmBulkUpdate = async () => {
  try {
    ...
    await etsyYunTuDeductionAPI.bulkUpdate({ ids, updates })
...
const downloadTemplate = async () => {
  try {
    const response = await etsyYunTuDeductionAPI.downloadTemplate()
    const blob = new Blob([response.data])
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = '云图扣费表模板.xlsx'
    link.click()
...
const handleImport = async () => {
  ...
  try {
    ...
    await etsyYunTuDeductionAPI.importData(file)
...
// 获取扣费类型标签类型
const getDeductionTypeType = (deductionType: string) => {
  const deductionTypeMap: Record<string, string> = {
    '图片扣费': 'primary',
    '视频扣费': 'success',
    '模板扣费': 'warning'
  }
  return deductionTypeMap[deductionType] || 'info'
}

// Redis缓存相关方法
const handleSyncData = async () => {
  try {
    await etsyYunTuDeductionAPI.syncData()
...
const handleClearAllCache = async () => {
  try {
    await etsyYunTuDeductionAPI.clearCache()
